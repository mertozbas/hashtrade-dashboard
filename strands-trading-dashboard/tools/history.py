...
def history(
    ...
    if action == "add":
        ...
        return {"status": "success", "content": [{"text": json.dumps(rec, ensure_ascii=False)}], "record": rec}

    if action == "tail":
        limit = int(limit or 200)
        raw_lines = _read_last_lines(HISTORY_FILE, max(1, limit))
        items = []
        for ln in raw_lines:
            ln = (ln or "").strip()
            if not ln:
            ...
            try:
                ...
                # keep raw
                items.append({"ts": None, "type": "raw", "turn_id": "", "data": {"line": ln}})
        return {"status": "success", "content": [{"text": json.dumps(items, ensure_ascii=False)}], "items": items}
